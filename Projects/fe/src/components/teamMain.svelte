<script>
  ...
  import Aojh from "../images/ao 제희형님.jpg";
  import Aoce from "../images/ao신초은.jpg";
  import Aoyn from "../images/ao최윤아.jpg";
  import SjIcon from "../images/임시.jpg";
  import JhIcon from "../images/임시2.png";
  import CeIcon from "../images/임시3.png";
  import YnIcon from "../images/임시4.jpg";
  ...
  function showChange() {
    ...
    if (showImg == AoJc) {
      ...
      showImg = JhIcon;
    } else if (showImg == JhIcon) {
      showImg = Aojh;
    } else if (showImg == Aoce) {
      showImg = CeIcon;
    } else if (showImg == CeIcon) {
      showImg = Aoce;
    } else if (showImg == Aoyn) {
      showImg = YnIcon;
    } else if (showImg == YnIcon) {
      showImg = Aoyn;
    }

    //showImg = isFlipped ? IconPhoto : ImgChange();
  }

  function JcChangeImg() {
    showImg = AoJc;
    checkNum.set(1);
    isFlipped = false;
  }

  function SjchangeImage() {
    showImg = Aosj;
    checkNum.set(2);
    isclick = false;
    isFlipped = false;
  ...
  function JhchangeImage() {
    showImg = Aojh;
    checkNum.set(3);
    isFlipped = false;
  }

  function CechangeImage() {
    showImg = Aoce;
    checkNum.set(4);
    isFlipped = false;
  }

  function YnchangeImage() {
    showImg = Aoyn;
    checkNum.set(5);
    isFlipped = false;
  }
...
<div
  class="flex flex-col justify-center items-center h-[338px] gap-2.5 w-screen bg-[#E3E3E3] dark:bg-[#EEE] rounded-tl-lg rounded-tr-lg"
  style="border-top-left-radius: 30px; border-top-right-radius: 30px;"
>
  <div class="w-[131px] h-[129px] absolute top-[190px] mr-[180px]">
    <svg
      width="86"
      height="86"
      viewBox="0 0 86 86"
      ...
      class="w-[86px] h-[86px] absolute left-[45px] top-0"
    ...
    <button on:click={showChange}>
      <img
        src={showImg}
        alt="메인 사진"
        class="shadow-xl w-[99px] h-[99px] absolute left-[2px] top-[27px] rounded-tl-[100px] rounded-tr-[3px] rounded-bl-[100px] rounded-br-[100px] object-cover animate-bounce
        {isFlipped ? 'flipped' : ''}"
  ...
  {#if $checkNum == 1}
    <div
      class="flex flex-col justify-center items-start gap-[18px] ml-[50px] mt-[40px]"
    >
      <div
        class="flex flex-col justify-center items-start flex-grow-0 flex-shrink-0 relative"
      >
        <p
          class="flex-grow-0 flex-shrink-0 w-auto text-[13px] font-['PRETENDARD-BOLD'] text-left text-black"
        >
          배종찬
        </p>
        <p
    ...
    <div
      class="flex flex-col justify-center items-center w-[87%] relative overflow-hidden px-[25px] py-[28px] rounded-[20px] bg-white mt-[30px]"
      style="box-shadow: 0px 0px 10px 1px rgba(0,0,0,0.1);"
    >
      <p
        class="font-['PRETENDARD-MEDIUM'] gap-[3px] flex flex-col flex-grow-0 flex-shrink-0 text-[10px] text-center text-black"
      >
        ...
        >
        <span
          >저희 AO 스터디에서 바라왔던 대학생활 보내시면 좋겠습니다 🔥🔥”
        </span>
      </p>
    </div>
  {:else if $checkNum == 2}
    <div
      class="flex flex-col justify-center items-start relative gap-[18px] ml-[50px] mt-[40px]"
    >
      <div
        class="flex flex-col justify-center items-start flex-grow-0 flex-shrink-0 relative"
      >
    ...
    <div
      class="flex flex-col justify-center items-center w-[87%] relative overflow-hidden px-[24px] py-[28px] rounded-[20px] bg-white mt-[30px]"
      style="box-shadow: 0px 0px 10px 1px rgba(0,0,0,0.1);"
    >
      <p
        class="font-['PRETENDARD-MEDIUM'] gap-[3px] flex flex-col flex-grow-0 flex-shrink-0 text-[10px] text-center text-black"
      >
        ...
        <span>A(lcohol)O(nly)는 </span>
        <span
          >여러분 인생에서 처음일 수 있는 신입생 생활을 책임져 드립니다!!</span
        >
        <span
  ...
  {:else if $checkNum == 3}
    ...
    <div
      class="flex flex-col justify-center items-center w-[87%] relative overflow-hidden px-[25px] py-[28px] rounded-[20px] bg-white top-[40px]"
      style="box-shadow: 0px 0px 10px 1px rgba(0,0,0,0.1);"
    >
      <p
        class="font-['PRETENDARD-MEDIUM'] gap-[3px] flex flex-col flex-grow-0 flex-shrink-0 text-[10px] text-center text-black"
  ...
  {:else if $checkNum == 5}
    ...
    <div
      class="flex flex-col justify-center items-center w-[87%] relative overflow-hidden px-[20px] py-[28px] rounded-[20px] bg-white top-[40px]"
      style="box-shadow: 0px 0px 10px 1px rgba(0,0,0,0.1);"
    >
      <p
        class="font-['PRETENDARD-MEDIUM'] gap-[3px] flex flex-col flex-grow-0 flex-shrink-0 text-[10px] text-center text-black"
...
<div class="flex mt-[23px]">
  <p
    class="w-[159px] h-[22px] text-base font-semibold absolute text-left text-black left-[31px] dark:text-neutral-50"
  >
    AO Team Members
  </p>
</div>
...
<div class="flex justify-center items-center relative mt-10">
  <div class="flex-grow-0 flex-shrink-0 w-[109px] h-[119px] relative">
    <div class="w-[79px] h-[75px] absolute left-[15px] top-0">
      <div
        class="w-[75px] h-[75px] absolute left-[-1px] top-[-1px] rounded-tl-[100px] rounded-tr-[3px] rounded-bl-[100px] rounded-br-[100px] bg-[#eee]"
      ></div>
      <button on:click={SjchangeImage}>
        <img
          src={Aosj}
          alt="세진형님 사진"
          class="w-[71px] h-[71px] absolute left-[1.44px] top-[1.44px] rounded-tl-[100px] rounded-tr-[3px] rounded-bl-[100px] rounded-br-[100px] object-cover
    ...
    <div
      class="flex flex-col justify-center items-center absolute left-0 top-[87px] gap-px"
    >
      <p
        class="flex-grow-0 flex-shrink-0 w-[109px] text-xs font-bold text-center text-black dark:text-neutral-50"
      >
        박세진
      </p>
      <p
...
<p
  class="w-[159px] h-[22px] text-base font-semibold text-left text-black mt-[50px] ml-[31px] dark:text-neutral-50"
>
  AO Web Developers
</p>

<div class="w-64 h-[106px] relative ml-[60px] mt-[29px]">
  <div class="w-64 h-[25px] absolute left-0 top-0">
    <p
